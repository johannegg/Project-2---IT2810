describe('View a song', () => {
    beforeEach( () => {
      ...
      it('should show a song when you click on it, and then go back', () => {
        ...
        cy.get('@selectedRow').find('.titleCell').invoke('text').then((title) => {
    ...
    it('should add song to favorite from song page', () => {
      ...
      cy.get('.tableRow').find('.titleCell').invoke('text').then((title) => {
        expect(title).to.exist;
        cy.contains(title).should('be.visible');
    ...
    it.only('should add song to playlist from song page', () => {
      // Log in first
      cy.get('.profile-icon').click();
      cy.get('.login-input').type('test');
      cy.get('.login-button').click();
      ...
      cy.get('.tableRow').eq(4).find('.titleCell').invoke('text').then((title) => {
        expect(title).to.exist;
